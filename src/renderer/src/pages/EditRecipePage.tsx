      ...
      <div className="create-material-page">
        ...
        <div className="form-container">
          <div className="server-error" role="alert">
            {loadError || t('recipes.edit.errors.loadFailed')}
          </div>
      ...
      <div className="page-header">
        <div className="page-header-left">
          ...
          <h2 className="page-title">
            ...
            <span className="edit-material-name">: {recipe.name}</span>
          </h2>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)} noValidate>
        ...
        <div className="form-container">
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <select id="yieldUnit" className="form-input form-select" {...register('yieldUnit')}>
        ...
        <div className="form-container edit-section">
          ...
          {ingredients.length === 0 ? (
            <p className="empty-text">{t('recipes.view.noIngredients')}</p>
          ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td>
                        {isEditing ? (
                          <select
                            ...
                            onChange={(e) => setEditIngredientUnit(e.target.value as 'kg' | 'g')}
                          >
                      ...
                      <td className="actions-col">
                        {isEditing ? (
                          <>
                            <button type="button" className="btn-icon btn-save" onClick={() => handleSaveIngredient(ingredient.id)} title={t('common.save')}>&#10003;</button>
                            <button type="button" className="btn-icon" onClick={handleCancelEditIngredient} title={t('common.cancel')}>&#10005;</button>
                          </>
                        ) : (
                          <>
                            <button type="button" className="btn-icon" onClick={() => handleStartEditIngredient(ingredient)} title={t('common.edit')}>&#9998;</button>
                            <button type="button" className="btn-icon btn-danger" onClick={() => handleRemoveIngredient(ingredient.id)} title={t('common.delete')}>&#128465;</button>
                          </>
                        )}
                      </td>
                    </tr>
        ...
        <div className="form-container edit-section">
          ...
          {packaging.length === 0 ? (
            <p className="empty-text">{t('recipes.view.noPackaging')}</p>
          ) : (
            <table className="edit-table">
              <thead>
                <tr>
              ...
              <tbody>
                {packaging.map((pkg) => {
                  const lineCost = calculatePackagingCost(pkg.quantity, pkg.packagingMaterial.unitPrice)
                  ...
                  return (
                    <tr key={pkg.id}>
                      ...
                      <td className="actions-col">
                        {isEditing ? (
                          <>
                            <button type="button" className="btn-icon btn-save" onClick={() => handleSavePackaging(pkg.id)} title={t('common.save')}>&#10003;</button>
                            <button type="button" className="btn-icon" onClick={handleCancelEditPackaging} title={t('common.cancel')}>&#10005;</button>
                          </>
                        ) : (
                          <>
                            <button type="button" className="btn-icon" onClick={() => handleStartEditPackaging(pkg)} title={t('common.edit')}>&#9998;</button>
                            <button type="button" className="btn-icon btn-danger" onClick={() => handleRemovePackaging(pkg.id)} title={t('common.delete')}>&#128465;</button>
                          </>
                        )}
                      </td>
                    </tr>
        ...
        {/* Cost Summary */}
        <div className="form-container cost-summary">
          <h3 className="section-title">{t('recipes.view.costBreakdown')}</h3>
          <div className="cost-grid">
            <span>{t('recipes.view.materialCost')}:</span>
            <span className="text-right">{formatPrice(ingredientsCost)}</span>
            <span>{t('recipes.view.packagingCost')}:</span>
            <span className="text-right">{formatPrice(packagingCost)}</span>
            <span className="total-label">{t('recipes.view.totalCost')}:</span>
            <span className="text-right total-value">{formatPrice(totalCost)}</span>
          </div>
        </div>
      ...
      {showAddPackaging && (
        <div className="modal-overlay">
          <div className="modal">
            ...
            {addPackagingError && (
              <div className="server-error" role="alert">{addPackagingError}</div>
            )}

            <div className="form-group">
              <label htmlFor="packaging-select" className="form-label">{t('recipes.edit.packaging.selectPackaging')}</label>
              <select
                id="packaging-select"
                className="form-input form-select"
            ...
            <div className="form-group">
              <label htmlFor="packaging-quantity" className="form-label">{t('recipes.view.quantity')}</label>
              <input
